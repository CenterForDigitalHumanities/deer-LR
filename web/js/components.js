...
class LrNav extends HTMLElement {
    constructor() {
        super()
        document.querySelector('lr-nav').addEventListener('lrUserKnown', event => {
    ...
    constructor() {
        ...
        if (user !== null) {
            try {
                ...
                document.dispatchEvent(new CustomEvent('lrUserKnown', { detail: { user: user }, composed: true, bubbles: true }))
            } catch (err) {
        ...
        try {
            this.querySelector('FORM').onsubmit = async function(event) {
                ...
                if (authenticatedUser && authenticatedUser["@id"]) {
                    document.dispatchEvent(new CustomEvent('lrUserKnown', { detail: { user: authenticatedUser } }))
